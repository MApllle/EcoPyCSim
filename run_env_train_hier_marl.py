...
def save_run_snapshot(env_dir, hyper_params):
    ...
    files_to_copy = [
        "run_env_train_hier_marl.py",
        "env/cloud_scheduling_hier.py",
        "schedulers/marl/hier_marl/HierMARL.py",
    ]
...
num_server_farms = int(os.getenv("NUM_FARMS",  "2"))
num_servers      = int(os.getenv("NUM_SERVERS","6"))  # must be divisible by num_server_farms
...
for episode in range(episode_num):
    ...
    while True:
        ...
        sel_farm = info["global"]["selected_farm_id"]
        hier.add(obs, action, reward, next_obs, done, sel_farm)
    ...
    print(
        f"[HierMARL] ep {episode + 1:3d}/{episode_num}  "
        f"eps={epsilon:.3f}  "
    ...
    hier.save(episode_global_rewards)
...
axes[0].set_title('HierMARL — Global Agent')
axes[0].legend()
...
axes[1].set_title('HierMARL — Local Agents (average)')
axes[1].legend()

plt.tight_layout()
title = 'HierMARL performance'
