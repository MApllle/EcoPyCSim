"""VDN 单体 Agent —— 只包含 Q-network，不含独立优化器。

与 IDQN 的 DQNAgent 相比：
- **保留**：MLPNetwork 结构、target_q_net（软更新）、select_action（epsilon-greedy）、soft_update
- **删除**：self.optimizer 和 learn() ——优化器由 VDN 协调器统一管理，
           梯度通过 Q_tot = Σ Q_i 的求和操作自动流回此网络
- **新增**：q_values() / target_q_values() ——纯前向接口，供协调器调用
"""
...
class MLPNetwork(nn.Module):
    """与 MADDPG / IDQN 保持相同的 2 层 MLP 结构，方便公平对比。"""
...
class VDNAgent:
    ...
    VDN 中所有 agent 的 Q-network 参数由协调器（VDN）统一放入一个 Adam optimizer，
    通过联合损失 MSE(Q_tot, target) 同步更新，实现集中训练。
    ...
    # ------------------------------------------------------------------
    # 前向接口（供 VDN 协调器在 learn() 中调用）
    # ------------------------------------------------------------------

    def q_values(self, obs: torch.Tensor) -> torch.Tensor:
        """返回所有动作的 Q 值，shape (B, act_dim)。用于计算 Q_tot。"""
        return self.q_net(obs)

    def target_q_values(self, obs: torch.Tensor) -> torch.Tensor:
        """返回目标网络的 Q 值，shape (B, act_dim)。用于计算 TD target。"""
