...
class CloudSchedulingEnv(ParallelEnv):
  ...
  def _take_action(self, actions):
    ...
    if sid in server_farm.servers:
        server = server_farm.servers[sid]
    elif int(sid) in server_farm.servers:
        server = server_farm.servers[int(sid)]
    else:
        # 打印一下当前的可用 Key，方便调试
        print(f"Available keys: {list(server_farm.servers.keys())}, Looking for: {sid}")
        # 也可以给一个默认值
        server = list(server_farm.servers.values())[0]
  ...
  def _get_reward(self, agent):
    ...
    if not self.task_rejected_status:
      task_success_reward = 5 # Positive reward for successful task scheduling
    else:
      task_success_reward = -60  # Penalty for task rejection
