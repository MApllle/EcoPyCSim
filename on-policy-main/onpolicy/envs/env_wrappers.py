...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ob, s_obs, reward, done, info, avail_act = env.step(data)
            ...
            remote.send((ob, s_obs, reward, done, info, avail_act))
        elif cmd == 'reset':
            ob, s_obs = env.reset()
            remote.send((ob, s_obs))
        elif cmd == 'render':
            if data == "rgb_array":
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs = np.stack([res[0] for res in results])
        share_obs = np.stack([res[1] for res in results])
        rews = np.stack([res[2] for res in results])
        dones = np.stack([res[3] for res in results])
        infos = [res[4] for res in results]
        return obs, share_obs, rews, dones, infos, None
    ...
    def reset(self):
        ...
        # results 现在是从 8 个进程收到的 [(obs, s_obs, info), (obs, s_obs, info), ...]
        results = [remote.recv() for remote in self.remotes]

        # 1. 拆解并堆叠 local_obs (形状变为 [8, 2, 101])
        obs = np.stack([res[0] for res in results])

        # 2. 拆解并堆叠 share_obs (形状变为 [8, 2, 202])
        share_obs = np.stack([res[1] for res in results])

        # 3. 收集 infos (这是一个包含 8 个字典的列表)
        # infos = [res[2] for res in results]

        # 3. 关键修改：手动构造一个空的 infos 列表，不要去 res 里找 index 2
        infos = [{} for _ in range(self.num_envs)]

        # 返回 3 个值，完美适配 runner.py 的 obs, share_obs, infos = self.envs.reset()
        return obs, share_obs, infos
        # 返回 3 个值，完美适配 runner 的接头
        # return obs, share_obs, infos
        # return obs, share_obs, [{} for _ in range(self.num_envs)] # 补一个空的 info 列表给 runner
...
class DummyVecEnv(ShareVecEnv):
    ...
    def reset(self):
        ...
        # 如果是评估模式，可能包含非矩阵数据，强制转为 object 类型
        try:
            return np.array(obs)
        except ValueError:
